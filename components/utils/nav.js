    ...
    <nav className="w-fit p-2 rounded-full dark:bg-dark-secondary bg-light-primary dark:ring-dark-green-1 dark:drop-shadow-customDark  ring-1 ring-dark-green-1/10 drop-shadow-customLight">
      <ul className="flex justify-center items-center">
        {navItems.map((item, index) => (
          <li key={index} className={`${item.href.toLowerCase() != currentPath?"hover:scale-[1.08] transition duration-200 ease-out hover:ease-in":""}`}>
            <Link href={item.href}>
              <a className="relative dark:text-dark-1 text-slate-800  px-3 py-3 dark:font-light">
                {item.name}
                <span
